...
const ImportSchema = z.object({
  url: z.string().url({ message: 'Please enter a valid book URL, e.g., https://www.69shu.tw/book/21.html' }),
});
...
export async function importStory(formData) {
  ...
  try {
    ...
    const coverImage = $(selectors.coverImage).src;

    if (!title || !synopsis) {
      return { error: 'Failed to extract story title or synopsis from the book page.' };
    }
    ...
    const story = await db.story.create({
      data: {
        ...
        slug: translatedTitle.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, ''),
        status: 'DRAFT',
        genres: [],
      },
    });
    ...
    for (let i = 0; i < chapterLinks.length; i++) {
      ...
      const [translatedChapterTitle, translatedChapterContent] = await Promise.all([
    ...
    return { success: `Story "${translatedTitle}" imported successfully as a draft!` };
